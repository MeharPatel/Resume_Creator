...
export const ResumePreview = ({ formData, selectedTemplate, setSelectedTemplate }) => {
  const { toPDF, targetRef } = usePDF({
    ...
    page: {
      margin: { top: 15, right: 0, bottom: 20, left: 0 }, // Consistent margins in mm
      format: 'a4',
    },
    resolution: 2,
  });
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      >
            ...
            {formData.education.map((edu, index) => (
              <div key={index} className="mt-2 ">
                ...
                <p>
                  ...
                  {edu.description && (
                  <span className={template.itemDescription}> | {edu.description}</span>
                )}
                </p>
              </div>
            ))}
        ...
        {formData.projects.length > 0 && (
          <section className='keep-together no-break'>
            ...
            {formData.projects.map((proj, index) => (
              ...
              <div className='flex'>
                ...
                <h3 className={template.itemTitle}>{proj.title}</h3>
                  {proj.projectUrl.length > 0 && (<span className={`${template.itemSubtitle} ms-2 mt-1`}> | {proj.projectUrl}</span>)}
              </div>
...
//             whileTap={{ scale: 0.95 }}
//             onClick={() => setSelectedTemplate(templateKey)}
//             className={`px-4 py-2 rounded-sm capitalize ${selectedTemplate === templateKey ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
//           >
//             {templateKey}
//           </motion.button>
//         ))}
...
//           className={`${template.resume_container} pdf-container`}
//         >
//           <h1 className={`${template.name} keep-together`}>{formData.firstName} {formData.lastName}</h1>
//           <div className="keep-together">
//             <div className={template.itemSubtitle}>
//               <div className="flex flex-wrap gap-2">
//                 {formData.email && <span className="flex items-center">
//                 {/* <AtSign className={`${template.custom_icons} h-4 w-4 mr-1`} /> */}
//                  {formData.email}</span>}
//                 {formData.phone && <span className="flex items-center"><span className="mx-1">|</span>
...
//               <p className={template.itemSubtitle}>{formData.professional_title} {formData.professional_description && ` | ${formData.professional_description}`}</p></div>
//             )}
...
//               <p className={template.itemSubtitle}>{[formData.address, formData.city, formData.state, formData.country, formData.zip_code].filter(Boolean).join(', ')}</p></div>
//             )}
//           </div>
//           {(formData.socialLinks.github || formData.socialLinks.linkedin || formData.socialLinks.instagram || formData.socialLinks.facebook || formData.socialLinks.portfolio) && (
//             <div className="keep-together">
//               <div className="grid grid-cols-2 gap-1 mb-4">
//                 {formData.socialLinks.github && <div className="flex">
//                 {/* <Github className={`${template.custom_icons} h-4 w-4 mr-2 mt-1`} /> */}
//                 <a href={formData.socialLinks.github} className={template.itemSubtitle}>{formData.socialLinks.github}</a></div>}
//                 {formData.socialLinks.linkedin && <div className="flex">
//                 {/* <Linkedin className={`${template.custom_icons} h-4 w-4 mr-2 mt-1`} /> */}
//                 <a href={formData.socialLinks.linkedin} className={template.itemSubtitle}>{formData.socialLinks.linkedin}</a></div>}
//                 {formData.socialLinks.instagram && <div className="flex">
//                 {/* <Instagram className={`${template.custom_icons} h-4 w-4 mr-2 mt-1`} /> */}
//                 <a href={formData.socialLinks.instagram} className={template.itemSubtitle}>{formData.socialLinks.instagram}</a></div>}
//                 {formData.socialLinks.facebook && <div className="flex">
//                 {/* <Facebook className={`${template.custom_icons} h-4 w-4 mr-2 mt-1`} /> */}
//                 <a href={formData.socialLinks.facebook} className={template.itemSubtitle}>{formData.socialLinks.facebook}</a></div>}
//                 {formData.socialLinks.portfolio && <div className="flex">
//                 {/* <Signature className={`${template.custom_icons} h-4 w-4 mr-2 mt-1`} /> */}
//                 <a href={formData.socialLinks.portfolio} className={template.itemSubtitle}>{formData.socialLinks.portfolio}</a></div>}
//               </div>
//             </div>
//           )}
//           {formData.education.length > 0 && (
...
//                   <h3 className={template.itemTitle}>{edu.degree} ({edu.field_of_study})</h3></div>
//                   <p className={template.itemSubtitle}>{edu.school} | {edu.start_date} - {edu.end_date}</p>
...
//                   <h3 className={template.itemTitle}>{exp.company}</h3></div>
//                   <p className={template.itemSubtitle}>{exp.position} | {exp.start_date} - {exp.end_date ? exp.end_date : 'Present'}</p>
...
//               <div className="flex flex-wrap gap-2">
//                 {formData.skills.map((skill, index) => (
//                   <span key={index} className={template.skill}>{skill.name} - {skill.level}</span>
//                 ))}
//               </div>
//             </section>
//           )}
...
//                   <h3 className={template.itemSubtitle}>{achievement.title}</h3></div>
//                 </div>
//               ))}
//             </section>
//           )}
...
//                   <h3 className={template.itemTitle}>{proj.title}</h3></div>
//                   {proj.projectUrl && <p className={template.itemSubtitle}><a href={proj.projectUrl} className="underline">{proj.projectUrl}</a></p>}
//                   <p className={template.itemDescription}>{proj.description}</p>
//                 </div>
//               ))}
//             </section>
...
//                   <div className="flex flex-wrap gap-2">
//                     {section.items.map((item, itemIndex) => (
//                       <span key={itemIndex} className={template.customSectionItems}>{item}</span>
//                     ))}
//                   </div>
//                 </div>
//               ))}
...
//       </div>
//       <div className="flex justify-center mt-8">
//         <button onClick={toPDF} className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition">Download PDF</button>
//       </div>
//     </div>
//   );
// };